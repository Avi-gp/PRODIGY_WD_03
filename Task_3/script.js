...
let playerSign = "X";
let aiPlayerIcon = "far fa-circle";
let aiPlayerSign = "O";
let runBot = true;
...
function bot(runBot) {
  let array = [];
  if (runBot) {
    playerSign = aiPlayerSign;

    for (let i = 0; i < allBox.length; i++) {
      if (allBox[i].childElementCount === 0) {
        allBox[i].innerHTML = `<i class="${aiPlayerIcon}"></i>`;
        allBox[i].setAttribute("id", aiPlayerSign);
    ...
    if (array.length > 0) {
      allBox[randomBox].innerHTML = `<i class="${aiPlayerIcon}"></i>`;
      allBox[randomBox].setAttribute("id", aiPlayerSign);
      selectWinner();
    }
